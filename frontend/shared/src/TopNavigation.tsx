...
export function TopNavigation(props: Props) {
  ...
  const [isBurgerMenuOpen, toggleBurgerMenu] = React.useState(false);
  const handleBurgerMenuClick = () => toggleBurgerMenu(!isBurgerMenuOpen);

  const [isProfileDropdownOpen, toggleProfileDropdown] = React.useState(false);
  const handleProfileDropdownClick = () => toggleProfileDropdown(!isProfileDropdownOpen);

  const removeIf = (isHidden: boolean) => (e: React.TransitionEvent) => {
  ...
  return (
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center">
              ...
              <div className="flex-shrink-0">
                <svg className="h-8 w-8" fill="none" viewBox="0 0 35 32" xmlns="http://www.w3.org/2000/svg">
                  <path
                    fill="#6875F5"
            ...
            <div className="hidden md:block">
              ...
              --> */}
                  <div
                    id="profile-dropdown"
                    onTransitionEnd={removeIf(!isProfileDropdownOpen)}
                    className={`origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg transition-all ease-out duration-100 ${
                      isProfileDropdownOpen ? "transform opacity-100 scale-100" : "transform opacity-0 scale-95"
                    }`}
                  >
                    <div
    ...
    --> */}
        <div
          id="mobile-menu"
          onTransitionEnd={removeIf(!isBurgerMenuOpen)}
          className={`md:hidden overflow-hidden transition-height ease-out duration-200 ${
            isBurgerMenuOpen ? "max-h-screen" : "max-h-0"
          }`}
        >
      ...
      {/* TODO: incorporate in the new markup */}
